function Build(settings, patterns) {
  ...
  for (let name in resources.settings.breakPoints) {
    str +=
      ...
      name +
      "{{=}}" +
      resources.settings.breakPoints[name].width +
      "{{;}}\n"
  }
  ...
  str += "{{var}}offset_one_side{{=}}calc({{var}}offset / 2){{;}}\n"

  for (let name in resources.settings.breakPoints) {
    str += `{{var}}offset_${name}_one_side{{=}}calc({{var}}offset_${name} / 2){{;}}\n`
  }
  ...
  for (let name in resources.settings.breakPoints) {
    size = new resources.size(
      resources,
      resources.settings.mixinNames.size,
      name
    )
    str += size.render() + "\n\n"
  }

  let shift = new resources.shift(
    resources,
    resources.settings.mixinNames.shift
  )
  str += shift.render() + "\n\n"

  for (let name in resources.settings.breakPoints) {
    shift = new resources.shift(
      resources,
      resources.settings.mixinNames.shift,
      name
    )
  ...
  for (let name in resources.settings.breakPoints) {
    let media = new resources.media(
      "{{var}}break_" + name,
      globalMediaCondition,
      resources.settings.defaultMediaDevice
    )

    let styles = "{{string-var}}name{{/string-var}}{{:}}{{var}}value{{;}}\n"
    let mix = new resources.mixin(
      resources.patterns.mixin,
      name,
      "{{var}}name, {{var}}value",
      media.wrap(styles, 1)
    )
  ...
  let fromMedia = new resources.media(
    "{{var}}min_width",
    "min-width",
    resources.settings.defaultMediaDevice
  )
  let fromCommand = "{{block-content-extract}}{{;}}\n"
  let from = new resources.mixin(
    resources.patterns.mixin,
    resources.settings.mixinNames.from,
    "{{var}}min_width{{block-content-var-delimeter}}{{block-content-var}}",
    fromMedia.wrap(fromCommand, 1)
  )
  str += from.render()
  str += "\n\n"

  let toMedia = new resources.media(
    "{{var}}max_width",
    "max-width",
    resources.settings.defaultMediaDevice
  )
  let toCommand = "{{block-content-extract}}{{;}}\n"
  let to = new resources.mixin(
    resources.patterns.mixin,
    resources.settings.mixinNames.to,
    "{{var}}max_width{{block-content-var-delimeter}}{{block-content-var}}",
    toMedia.wrap(toCommand, 1)
  )
  str += to.render()
  str += "\n\n"

  let fromTo = new resources.mixin(
    resources.patterns.mixin,
    resources.settings.mixinNames.fromTo,
    "{{var}}min_width, {{var}}max_width{{block-content-var-delimeter}}{{block-content-var}}",
    resources.patterns.fromTo
  )
  str += fromTo.render()
  str += "\n\n"
  ...
  let uSizeCommands = resources.styles.objToStyles(
    {
      width:
        "{{i}}calc(100% / {{string-var}}columns{{/string-var}} * {{string-var}}n{{/string-var}} - {{string-var}}offset{{/string-var}}){{/i}}",
    },
    1
  )

  let usize = new resources.mixin(
    resources.patterns.mixin,
  ...
  let reset = new resources.mixin(
    resources.patterns.mixin,
    resources.settings.mixinNames.reset,
    "",
    resources.patterns.reset
  )
  str += reset.render()
  str += "\n\n"

  let debug = new resources.mixin(
    resources.patterns.mixin,
    resources.settings.mixinNames.debug,
    "{{var}}background, {{var}}outline",
    resources.patterns.debug
  )
  str += debug.render()
  str += "\n\n"

  let clearfix = new resources.mixin(
    resources.patterns.mixin,
    resources.settings.mixinNames.clearfix,
    "",
    resources.patterns.clearfix
  )
  str += clearfix.render()
