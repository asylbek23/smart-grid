  ...
  render() {
    ...
    for (let i = 0; i < this.map.length; i++) {
      ...
      if (this.resources.helpers.isPercentage(this.resources.settings.offset)) {
        for (let j = 0; j < this.map[i].props.length; j++) {
          ...
          styles += this.resources.styles.objToCallMedia(this.postfix, {
            [this.map[i].props[
              j
            ]]: `{{var}}atom * {{var}}n + {{var}}offset${mediaPostfix}_one_side`,
          })
        }
        ...
        for (let j = 0; j < this.map[i].props.length; j++) {
          ...
          styles += this.resources.styles.objToCallMedia(this.postfix, {
            [this.map[i].props[
              j
            ]]: `{{i}}calc({{string-var}}val{{/string-var}} + {{string-var}}offset${mediaPostfix}_one_side{{/string-var}}){{/i}}`,
          })
        }
      }
      ...
      let mixin = new this.resources.mixin(
        ...
        this.name + this.map[i].name,
        "{{var}}n",
        new this.resources.media().wrap(styles, 1)
      )
